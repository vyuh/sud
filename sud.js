...
(function(name, definition) {
    ...
    function scope_gen(n) {
        ...
        for (i = 0; i < l; i++) {
            ...
            for (j = 0; j < n * n; j++) {
                scope[r * n * n + j] = true;
                scope[j * n * n + c] = true;
                scope[(gr + Math.floor(j / n)) * n * n + gc + j % n] = true;
            }
            scope[i] = false;
            var eye = [];
            for (j = 0; j < l; j++) if (scope[j]) {
                eye.push(j);
            }
            out.push(eye);
        }
    ...
    var open = 16;
    var data = 15;
    var may_b = [ 16384, 8192, 4096, 2048, 1024, 512, 256, 128, 64 ];
    var clr = scope_gen(3);
    ...
    sud.prototype.copyIn = function(t) {
        this.left = t.left;
        for (var i = 0; i < 81; i += 1) this.i_v[i].copyIn(t.i_v[i]);
    };
    ...
    list.prototype.crook = function() {
        ...
        if (stata) {
            ...
            if ((pos = this.first()) >= 81) return 1; else {
